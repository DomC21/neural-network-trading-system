...
def get_technical_analysis(ticker: str, period: str = "6mo") -> Dict:
    ...
    gain = (delta > 0) * delta
    loss = (delta < 0) * -delta
    avg_gain = np.concatenate(([np.nan], np.convolve(gain, np.ones(14)/14, mode='valid')))
    avg_loss = np.concatenate(([np.nan], np.convolve(loss, np.ones(14)/14, mode='valid')))
...
async def analyze_stock(
    ...
    context = {
        ...
        "required_phrases": {
            "historical_high": f"52-week High: ${technical['historical_data'][-1]['High']:.2f}",
            "current_metrics": f"Current Price: ${technical['price']:.2f}, RSI: {technical['rsi']:.1f}",
            "sector_lead": f"Sector: {fundamental['sector']}, Industry: {fundamental['industry']}",
            "net_premium": f"Market Cap: ${fundamental['market_cap']/1e9:.1f}B, P/E: {fundamental['pe_ratio']:.1f}"
        }
    }
